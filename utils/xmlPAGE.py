...
class pageData:
    ...
    def get_cell_data(self, element):
        r = element.attrib.get("row")
        rs = element.attrib.get("rowSpan")
        c = element.attrib.get("col")
        cs = element.attrib.get("colSpan")
        corners = element.findall("".join([".//", self.base, "CornerPts"])) 
        if corners is not None:
            corners = np.array(corners, dtype=np.int)
        return (int(r), int(rs), int(c), int(cs), corners)
    ...
    def new_page(self, name, rows, cols):
        ...
        self.tree = ET.ElementTree(self.xml)
    ...
    def add_element(self, r_class, r_id, r_type, r_coords, parent=None, score=None):
        ...
        if score is not None:
            t_reg.attrib = {
                ...
                "custom": "".join(["structure {type:", r_type, ";}", " score {reg_score:", score,";}"]),
            }
        else:
            t_reg.attrib = {
                "id": "_".join([r_class, str(r_id)]),
